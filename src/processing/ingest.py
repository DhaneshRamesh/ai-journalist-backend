...
def _resolve_keywords(keywords: Optional[List[str]] = None) -> List[str]:
    ...
    return ["AI", "journalism", "startups"]  # Default keywords without demo context
...
def _fetch_articles_for_keyword(kw: str, per_kw_limit: int, unique_key: str, since_utc: datetime) -> List[Dict[str, Any]]:
    ...
    entries = []
    for entry in feed.entries[:min(per_kw_limit, len(feed.entries))]:
...
if __name__ == "__main__":
    ...
    # Azure PostgreSQL connection (replace with your credentials)
    ...
    result = run_recent_ingest(db, limit=10, keywords=["AI"])
    print(result)
    db.close()